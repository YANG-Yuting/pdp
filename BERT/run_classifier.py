        ...
        with open(input_file, encoding=encoding) as fin:
            for line in fin:
                ...
                if MR:
                    ...
                    # label, sep, text = line.partition(',')
                    label = line[-1]  # tiz: original ways to read is wrong
                    text = line[:-2]
...
def main():
    ...
    # tiz
    tokenizer = BertTokenizer.from_pretrained(os.path.join(args.bert_model, 'bert-base-uncased-vocab.txt'), do_lower_case=args.do_lower_case)
