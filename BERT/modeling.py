...
class BertSelfAttention(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        ...
        layer=nn.Linear(config.hidden_size, self.all_head_size)
        self.aux_qs = nn.ModuleList([copy.deepcopy(layer) for _ in range(args.num_models)])
        self.aux_ks = nn.ModuleList([copy.deepcopy(layer) for _ in range(args.num_models)])
        self.aux_vs = nn.ModuleList([copy.deepcopy(layer) for _ in range(args.num_models)])
        self.params_aux=None

    def transpose_for_scores(self, x):
        print(x.size())
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size) #(768,12,64)
        print(new_x_shape)
        x = x.view(*new_x_shape)

        return x.permute(0, 2, 1, 3)   #[1,12,128,64]

    def forward(self, hidden_states, attention_mask, head_mask=None,model_id=None,first_layer=False):
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)       # hidden_states  [batch_size, sequnce_length, embeding_dim]
        if first_layer:
            mixed_query_layer+=self.aux_qs[model_id](hidden_states)
            mixed_key_layer+=self.aux_k[model_id](hidden_states)
            mixed_value_layer+=self.aux_vs[model_id](hidden_states)

        query_layer = self.transpose_for_scores(mixed_query_layer) #[1,12,128,64]
        ...
        context_layer = torch.matmul(attention_probs, value_layer)   #[1,12,128 64]
        if self.keep_multihead_output:
            self.multihead_output = context_layer
            self.multihead_output.retain_grad()

        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()  #[1,128,12,64]
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)

        if first_layer:
            self.params_aux=attention_probs
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, head_mask=None, model_id=None,first_layer=False):
        self_output = self.self(input_tensor, attention_mask, head_mask,model_id,first_layer)
        if self.output_attentions:
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None,model_id=None,first_layer=False):
        attention_output = self.attention(hidden_states, attention_mask, head_mask,model_id,first_layer)
        if self.output_attentions:
...
class BertEncoder(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        ...
        layer = BertLayer(config, output_attentions=output_attentions,
                                  keep_multihead_output=keep_multihead_output)
        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(config.num_hidden_layers)])
        # self.aux = models.AuxModel()
        # self.params_aux=None

    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, head_mask=None,model_id=None):
        all_encoder_layers = []
        all_attentions = []
        for i, layer_module in enumerate(self.layer):
            if i==0:
                hidden_states = layer_module(hidden_states, attention_mask, head_mask[i],model_id,first_layer=True)
            else:
                hidden_states = layer_module(hidden_states, attention_mask, head_mask[i],model_id)
            if self.output_attentions:
                attentions, hidden_states = hidden_states
                # if i < 0:
                #     m = torch.zeros(hidden_states.size()[0], hidden_states.size()[1], 1, dtype=torch.float).cuda()
                #     m = m + model_ids
                #     x = torch.cat([hidden_states, m], dim=-1)
                #     self.params_aux = self.aux(x)  # [b_size, seq_len, hid_size]  - - - - - - -
                #     hidden_states += args.aux_weight * self.params_aux
                # if i==0:
                #     print(model_ids, x[:, :, -1])
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, speaker_ids=None,
                output_all_encoded_layers=True, head_mask=None,model_id=None):
        ...
        encoded_layers = self.encoder(embedding_output,
                                      ...
                                      head_mask=head_mask,model_id=model_id)
        if self.output_attentions:
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, head_mask=None,model_id=None, labels=None,):
        outputs = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, head_mask=head_mask,model_id=model_id)
