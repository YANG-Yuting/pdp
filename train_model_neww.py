...
def Loss_withEE_DPP(y_true, y_pred, num_model=args.num_models):
    ...
    if args.lamda == 0 and args.log_det_lamda == 0:
        ...
        return CE_all, torch.tensor(0.0), torch.tensor(0.0), torch.tensor(0.0)
    else:
...
def train_epoch(epoch, best_test, model, optimizer, dataloader_train, test_x, test_y, tokenizer):
    ...
    for idx, (*train_x, train_y) in enumerate(dataloader_train):
        ...
        if torch.cuda.device_count() > 1:
            ...
            for i in range(args.num_models):
                _, lg = model.module.models[i](input_ids, attention_mask=input_mask, token_type_ids=segment_ids)
                logits.append(lg)
            ...
            true_labels = []
            for i in range(args.num_models):
                _, lg = model.models[i](input_ids, attention_mask=input_mask, token_type_ids=segment_ids)
                logits.append(lg)
                true_labels.append(train_y)
            logits = torch.cat(logits, dim=0)
            true_labels = torch.cat(true_labels, dim=0)
    ...
    # For process 0
    if is_main_process() and epoch % 3 == 0:
        test_acc = eval_model(model, test_x, test_y)
        # test_acc = []
        # for i in range(args.num_models):
        #     test_acc.append(eval_model(model.module.models[i], test_x, test_y))
        time_end = time.time()
        time_used = time_end - time_start
        # sys.stdout.write("Epoch={} time={:.2f}s train_loss={:.6f} test_acc={:.6f}/{:.6f}/{:.6f}(#{})\n".format(
        #                     epoch, time_used, loss.item(), test_acc[0], test_acc[1], test_acc[2], len(test_y)))
        ...
        if test_acc > best_test:
            ...
            if torch.cuda.device_count() > 1:
                if args.target_model == 'bert':
                    if args.save_path:
                        for i in range(args.num_models):
                            save_path = args.save_path + '_%d' % i
                            if not os.path.exists(save_path):
                                os.makedirs(save_path)
                            torch.save(model.module.models[i].state_dict(), save_path + '/pytorch_model.bin')
                            model.module.models[i].config.to_json_file(save_path + '/bert_config.json')
                            # if args.task != 'snli':
                ...
                if args.target_model == 'bert':
                    if args.save_path:
                        for i in range(args.num_models):
                            ...
                            if not os.path.exists(save_path):
                                os.makedirs(save_path)
                            torch.save(model.models[i].state_dict(), save_path + '/pytorch_model.bin')
                            model.models[i].config.to_json_file(save_path + '/bert_config.json')
                            # if args.task != 'snli':
                            #     tokenizer.save_vocabulary(save_path)
                elif args.target_model == 'lstm':
    ...
    if is_main_process():
        if torch.cuda.device_count() > 1:
            for i in range(args.num_models):
                save_path = args.save_path + '_final' + '_%d' % i
...
def main(args):
    ...
    """对于mr-bert，关闭除了分类层以外的权重"""
    if args.task == 'mr' and args.target_model == 'bert':
        if torch.cuda.device_count() > 1:
            for i in range(args.num_models):
                model.module.models[i].bert.requires_grad_(False)
        else:
            for i in range(args.num_models):
                model.models[i].bert.requires_grad_(False)
    # if args.target_model == 'bert':
    #     if torch.cuda.device_count() > 1:
    #         if args.task == 'snli':
