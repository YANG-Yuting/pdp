...
def create_batches(x, y,max_len, batch_size, map2id, perm=None, sort=False, weight=None):
    ...
    for i in range(nbatch):
        bx, by = create_one_batch(x[i * size:(i + 1) * size], y[i * size:(i + 1) * size], max_len, map2id)
        batches_x.append(bx)
        batches_y.append(by)

        if weight != None:
            batches_weight.append(torch.tensor(weight[i * size:(i + 1) * size]))
...
def create_batches_x(x, max_len, batch_size, map2id, perm=None, sort=False):
    ...
    nbatch = (len(x)-1) // size + 1
    for i in range(nbatch):
        bx = create_one_batch_x(x[i*size:(i+1)*size], max_len, map2id)
        sum_len += len(bx)
        batches_x.append(bx)
