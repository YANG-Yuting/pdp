...
def attack(idx, text_ls, true_label, predictor, stop_words_set, word2idx, sim_predictor=None, import_score_threshold=-1.,
    ...
    if true_label != orig_label:
        ...
        for idx, synonyms in synonyms_all:
            it += 1
            if it > max_it:  # 达到最大替换个数，返回
                # 可能无替换(num_changed=0)，则说明样本周围不存在置信度降低的扰动，此时返回的是原样本(此时text_prime=text_ls)
            ...
            new_label_probs = new_probs[:, orig_label] + torch.from_numpy((semantic_sims < sim_score_threshold)
                                                                          + (1 - pos_mask).astype(float)).float().cuda()
            new_label_prob_min, new_label_prob_argmin = torch.min(new_label_probs, dim=-1)
            if new_label_prob_min < orig_prob:  # 当替换后置信度下降，返回
...
def main(data_x, data_y, split):
    ...
    with open(w_path, 'a+') as f:
        for idx, (text_word, true_label) in enumerate(zip(data_x, data_y)):  # for each data
            ...
            # 保存对抗样本
            outs[idx] = {'text': ' '.join(text_word), 'label': true_label, 'modify_ratio': modify_ratio, 'adv_texts':[new_text]}
            f.write(json.dumps(outs[idx]) + '\n')
            f.flush()
            e_time = time.time()
            print('idx:%d, time: %.2f, Acc: %.4f, Attack succ: %.4f, robust acc: %.4f' % ( idx, e_time-s_time,
            float(predict_true / len(data_y)), float(success_count) / float(predict_true),
